...
ReplayClientOptions ParseReplayClientOptions(int argc, char* const argv[], const unsigned default_cpu) {
    ...
    for (int index = 1; index < argc;) {
        ...
        if (index + 1 >= argc) {
            throw std::invalid_argument("client option is missing its value");
        }
        ...
        if (option == "--host") {
            ...
            if (value != "tcp" && value != "grpc" && value != "udp-multicast") {
            ...
            if (cpu > std::numeric_limits<unsigned>::max()) {
                throw std::invalid_argument("--cpu exceeds the supported range");
            }
            ...
            const std::uint64_t port = ParseUnsigned(value, option);
            if (port == 0 || port > std::numeric_limits<std::uint16_t>::max()) {
                throw std::invalid_argument("--md-a-port must be in [1, 65535]");
            }
            ...
            const std::uint64_t port = ParseUnsigned(value, option);
            if (port == 0 || port > std::numeric_limits<std::uint16_t>::max()) {
                throw std::invalid_argument("--md-b-port must be in [1, 65535]");
            }
            options.md_b_port = static_cast<std::uint16_t>(port);
        } else if (option == "--md-multicast-interface") {
            options.md_multicast_interface = value;
        } else {
            throw std::invalid_argument("unknown client option: " + std::string{option});
        }
